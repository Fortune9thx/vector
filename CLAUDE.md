...
- `gltest` direct-mode's WASI mock does **not** implement `GetTimestamp` yet, so `gl.vm.get_timestamp()` returns `None` in every direct-mode test — this would crash `_consensus_now()` in any direct-mode test that exercises a timestamp-touching path (bounty creation, `submit_disclosure`, `triage`, expiry). `tests/direct/conftest.py`'s `deploy_bounty()` catches this exact `AttributeError` and calls `pytest.skip()` with a clear reason, so CI reports these as skipped, not failed (56 of 77 as of this migration). This is a toolchain gap, not a contract bug — do not monkeypatch around it with a fake clock; time-dependent paths need an integration-network test until `gltest` catches up.
- A second, unrelated bug lived in the same file: `_find_real_address_cls()`'s version-agnostic glob for the SDK's `Address` class matched a stale pre-v0.3.0 cache entry first and polluted `sys.path`, breaking the very first contract import of any fresh `gltest` process (`No module named 'genlayer.types'`). Fixed by scoping the search to `extracted/local/` and the current `genlayer/types/__init__.py` path — see `SECURITY.md`.
